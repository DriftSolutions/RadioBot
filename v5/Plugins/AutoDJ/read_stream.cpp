...
bool read_stream_is_my_file(const char * fn) {
	if (!strncmp(fn,"stream://",9) || !strncmp(fn,"http://",7) || !strncmp(fn,"https://",8) || !strncmp(fn,"mmsh://",7)) {
		return true;
	}
	return false;
}
...
int64 read_stream_tell(READER_HANDLE * fp) {
...
int32 read_stream_read(void * buf, int32 size, READER_HANDLE * fp) {
	...
	if (hs->has_meta) {
		...
		if (i > 0) {
			n = ad_config.sockets->Recv(hs->sock,(char *)buf,i);
		...
		if ((hs->meta_ind % hs->bufsize) == 0) {
			unsigned char l = 0;
			if (ad_config.sockets->Recv(hs->sock,(char *)&l,1) == 1 && l) {
				char * str = new char[(l*16)+1];
				memset(str,0,(l*16)+1);
				ad_config.sockets->Recv(hs->sock,str,l*16);
		...
		if (hs->mmsh) {
			...
			if (n < size) {
				...
				while (n < size && ad_config.sockets->Recv(hs->sock, (char *)&pack, sizeof(pack)) == 4) {
					...
					if (pack.len) {
						...
						int n2=0, ind=0;
						int left = pack.len;
						while (left && (n2 = ad_config.sockets->Recv(hs->sock, buf2 + ind, left)) > 0) {
							ind += n2;
							left -= n2;
						}
						if (n2 >= 0) {
			...
			n = ad_config.sockets->Recv(hs->sock,(char *)buf,size);
			if (n <= 0) { fp->is_eof = true; return 0; }
			hs->pos += n;
		}
	}
...
int32 read_stream_write(void * buf, int32 size, READER_HANDLE * fp) {
	/*
	READER_HANDLE_STREAM * hs = (READER_HANDLE_STREAM *)fp->data;
	int n = ad_config.sockets->Send(hs->sock,(char *)buf,size);
	if (n <= 0) { fp->is_eof = true; return 0; }
	return n;
	*/
	return -1;
...
READER_HANDLE * read_stream_open(const char * fn, int32 mode) {
	if (!strstr(fn,"stream://") && !strstr(fn,"http://") && !strstr(fn,"https://") && !strstr(fn,"mmsh://")) { return NULL; }
	...
	bool https = strstr(fn,"https://") ? true:false;

	char buf[4096];
	sstrcpy(buf,fn);
	str_replace(buf,sizeof(buf),"stream://","");
	str_replace(buf,sizeof(buf),"http://","");
	str_replace(buf,sizeof(buf),"https://","");
	...
	if (!hs->port) {
		hs->port = https ? 443:80;
	}
	strlcpy(hs->url,p,sizeof(hs->url));

	hs->sock = ad_config.sockets->Create();
	if (!hs->sock) {
		delete hs;
		return NULL;
	}

	ad_config.sockets->SetSendTimeout(hs->sock, 5000);
	ad_config.sockets->SetRecvTimeout(hs->sock, 30000);
	api->ib_printf("AutoDJ (read_stream) -> Connecting to %s:%d ...\n", hs->host, hs->port);
	if (!ad_config.sockets->Connect(hs->sock,hs->host,hs->port)) {
		api->ib_printf("AutoDJ (read_stream) -> Error connecting to %s:%d (%s)\n", hs->host, hs->port, ad_config.sockets->GetLastErrorString(hs->sock));
		ad_config.sockets->Close(hs->sock);
		delete hs;
		return NULL;
	}

	ad_config.sockets->SetKeepAlive(hs->sock, true);

	if (https) {
		if (!ad_config.sockets->SwitchToSSL_Client(hs->sock)) {
			api->ib_printf("AutoDJ (read_stream) -> Error enabling TLS! (%s)\n", ad_config.sockets->GetLastErrorString(hs->sock));
			ad_config.sockets->Close(hs->sock);
			delete hs;
			return NULL;
		}
	...
	if (mmsh) {
		...
		sprintf(buf,"GET /%s HTTP/1.0\r\nConnection: close\r\nHost: %s:%d\r\nUser-Agent: Drift AutoDJ (Winamp 5.01)\r\nIcy-MetaData: 1\r\n\r\n", hs->url, hs->host, hs->port);
	}
	ad_config.sockets->Send(hs->sock,buf);
	...
	int n=0, line=0;
	while ((n = ad_config.sockets->RecvLine(hs->sock,buf,sizeof(buf))) >= RL3_NOLINE) {
		if (n == RL3_NOLINE) {
		...
		if (line == 1) {
			api->ib_printf("AutoDJ (read_stream) -> Response: %s\n", buf);
			if (!strstr(buf,"200")) {
				ad_config.sockets->Close(hs->sock);
				delete hs;
				zfree(ret);
			...
			if (p) {
				...
				if (!stricmp(buf,"icy-metaint")) {
					...
					if (hs->bufsize) {
						hs->has_meta = true;
						api->ib_printf("AutoDJ (read_stream) -> Has Icy MetaData every %d bytes...\n",hs->bufsize);
					}
				}
				if (!stricmp(buf,"content-type")) {
					sstrcpy(ret->mime_type, p);
					api->ib_printf("AutoDJ (read_stream) -> Mime Type: %s\n", p);
				}
			}
		}
	}
