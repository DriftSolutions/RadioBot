...
m_curlCallbackParamsSet( false )
{
    ...
    /* Initialize cURL */
    m_curlHandle = api->curl->easy_init();
...
twitCurl::~twitCurl()
{
    ...
    if( m_curlHandle )
    {
        api->curl->easy_cleanup( m_curlHandle );
        m_curlHandle = NULL;
    }
}
...
bool twitCurl::timelineHomeGet(std::string parms) {
    bool retVal = false;
    if( isCurlInit() )
    {
        /* Perform GET */
        retVal = performGet( twitterDefaults::TWITCURL_HOME_TIMELINE_URL + parms );
    }
    return retVal;
}
/*++
...
*
*--*/
bool twitCurl::mentionsGet(std::string parms)
{
    bool retVal = false;
    if( isCurlInit() )
    {
        /* Perform GET */
        retVal = performGet( twitterDefaults::TWITCURL_MENTIONS_URL + parms);
    }
    return retVal;
}
...
bool twitCurl::getUserInfoByID(int64 user_id)
{
	bool retVal = false;
	if (isCurlInit())
	{
		...
		/* Perform GET */
		retVal = performGet("https://api.twitter.com/1.1/users/show.json" + parms.str());
	}
	return retVal;
}

bool twitCurl::getUserInfoByScreenName(std::string screen_name)
{
	bool retVal = false;
	if (isCurlInit())
	{
		stringstream parms;
		parms << "?screen_name=" << urlencode(screen_name);
		/* Perform GET */
		retVal = performGet("https://api.twitter.com/1.1/users/show.json" + parms.str());
	}
	return retVal;
...
*
*--*/
bool twitCurl::directMessageGet(std::string parms)
{
    bool retVal = false;
    if( isCurlInit() )
    {
        /* Perform GET */
        retVal = performGet( twitterDefaults::TWITCURL_DIRECTMESSAGES_URL + parms);
    }
    return retVal;
}
...
*
*--*/
bool twitCurl::directMessageSend( uint64_t user_id, std::string& dMsg )
{
    bool retVal = false;
    if( isCurlInit() && user_id && dMsg.length() )
    {
		stringstream str;
		str << "{\"event\": {\"type\": \"message_create\", \"message_create\": {\"target\": {\"recipient_id\": \"" << user_id << "\"}, \"message_data\": {\"text\": \"" << escape_json(dMsg) << "\"}}}}";
        /* Perform POST */
        retVal = performPostJSON("https://api.twitter.com/1.1/direct_messages/events/new.json", str.str());        
    }
    return retVal;
}
...
void twitCurl::prepareCurlProxy()
{
    if( !m_curlProxyParamsSet )
    {
        ...
        //api->curl->easy_setopt( m_curlHandle, CURLOPT_PROXY, NULL );
        //api->curl->easy_setopt( m_curlHandle, CURLOPT_PROXYUSERPWD, NULL );
        //api->curl->easy_setopt( m_curlHandle, CURLOPT_PROXYAUTH, (long)CURLAUTH_ANY );

        ///* Set proxy details in cURL */
        //std::string proxyIpPort( "" );
        //utilMakeCurlParams( proxyIpPort, getProxyServerIp(), getProxyServerPort() );
        //api->curl->easy_setopt( m_curlHandle, CURLOPT_PROXY, proxyIpPort.c_str() );
        ...
        //    api->curl->easy_setopt( m_curlHandle, CURLOPT_PROXYUSERPWD, proxyUserPass.c_str() );
        //}
...
void twitCurl::prepareCurlCallback()
{
    if( !m_curlCallbackParamsSet )
    {
        ...
        api->curl->easy_setopt( m_curlHandle, CURLOPT_ERRORBUFFER, m_errorBuffer );

        /* Set callback function to get response */
        api->curl->easy_setopt( m_curlHandle, CURLOPT_WRITEFUNCTION, curlCallback );
        api->curl->easy_setopt( m_curlHandle, CURLOPT_WRITEDATA, this );
...
void twitCurl::prepareCurlUserPass()
{
    if( !m_curlLoginParamsSet )
    {
        ...
        api->curl->easy_setopt( m_curlHandle, CURLOPT_USERPWD, NULL );
        ...
        api->curl->easy_setopt( m_curlHandle, CURLOPT_USERPWD, userNamePassword.c_str() );
...
void twitCurl::prepareStandardParams()
{
    ...
    api->curl->easy_setopt( m_curlHandle, CURLOPT_CUSTOMREQUEST, NULL );
	api->curl->easy_setopt( m_curlHandle, CURLOPT_NOSIGNAL, 1 );
	api->curl->easy_setopt( m_curlHandle, CURLOPT_NOPROGRESS, 1);
	api->curl->easy_setopt( m_curlHandle, CURLOPT_TIMEOUT, 120 );
	api->curl->easy_setopt( m_curlHandle, CURLOPT_CONNECTTIMEOUT, 15 );
	api->curl->easy_setopt( m_curlHandle, CURLOPT_SSL_VERIFYHOST, 0 );
	api->curl->easy_setopt( m_curlHandle, CURLOPT_SSL_VERIFYPEER, 0);
	api->curl->easy_setopt( m_curlHandle, CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1);
...
bool twitCurl::performGet( const std::string& getUrl )
{
    ...
    {
        pOAuthHeaderList = api->curl->slist_append( pOAuthHeaderList, oAuthHttpHeader.c_str() );
        if( pOAuthHeaderList )
        {
            api->curl->easy_setopt( m_curlHandle, CURLOPT_HTTPHEADER, pOAuthHeaderList );
        }
    }

    /* Set http request and url */
    api->curl->easy_setopt( m_curlHandle, CURLOPT_HTTPGET, 1 );
    api->curl->easy_setopt( m_curlHandle, CURLOPT_URL, getUrl.c_str() );

    /* Send http request */
	CURLcode rret = api->curl->easy_perform( m_curlHandle );
    if( CURLE_OK == rret )
    {
        if( pOAuthHeaderList )
        {
            api->curl->slist_free_all( pOAuthHeaderList );
        }
        return true;
#if defined(DEBUG)
	} else {
		api->ib_printf("Twitter -> CURL error: %s\n", api->curl->easy_strerror(rret));
#endif
    }
    if( pOAuthHeaderList )
    {
...
bool twitCurl::performGet( const std::string& getUrl, const std::string& oAuthHttpHeader )
{
    ...
    /* Send http request */
    if( CURLE_OK == api->curl->easy_perform( m_curlHandle ) )
    {
        if( pOAuthHeaderList )
        {
            api->curl->slist_free_all( pOAuthHeaderList );
...
bool twitCurl::performDelete( const std::string& deleteUrl )
{
    ...
    /* Set http request and url */
    api->curl->easy_setopt( m_curlHandle, CURLOPT_CUSTOMREQUEST, "DELETE" );
    api->curl->easy_setopt( m_curlHandle, CURLOPT_URL, deleteUrl.c_str() );
...
bool twitCurl::performPost( const std::string& postUrl, std::string dataStr )
{
    ...
    api->curl->easy_setopt( m_curlHandle, CURLOPT_POST, 1 );
	char * surl = strdup(postUrl.c_str());
    api->curl->easy_setopt( m_curlHandle, CURLOPT_URL, surl );
#if defined(CURLOPT_COPYPOSTFIELDS)
    if( dataStr.length() )
    {
        api->curl->easy_setopt( m_curlHandle, CURLOPT_COPYPOSTFIELDS, dataStr.c_str() );
    }
#else
	char * datastr = NULL;
    if( dataStr.length() )
    {
		datastr = strdup(dataStr.c_str());
        api->curl->easy_setopt( m_curlHandle, CURLOPT_POSTFIELDS, datastr );
    }
#endif
...
bool twitCurl::performPostJSON(const std::string& postUrl, std::string dataStr)
{
	std::string oAuthHttpHeader("");
	struct curl_slist* pOAuthHeaderList = NULL;
	...
	pOAuthHeaderList = api->curl->slist_append(pOAuthHeaderList, "Content-Type: application/json");
