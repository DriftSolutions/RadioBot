...
void LoadMetaCache() {
	...
	strcpy(buf, "CREATE TABLE IF NOT EXISTS autodj_cache(FileID INTEGER PRIMARY KEY, mTime INTEGER, Title VARCHAR(255), Artist VARCHAR(255), Album VARCHAR(255), Genre VARCHAR(255));");
	...
	api->botapi->ib_printf("SimpleDJ -> Loaded meta DB in %u ticks\n", ticks);
#endif
...
//	char buf[512];
	...
	for (metaCacheList::const_iterator x = mList.begin(); x != mList.end(); x++) {
		if (!x->second->used) {
			...
			deleted++;
			if (deleted == 1) {
				api->botapi->ib_printf("SimpleDJ -> Deleting unused meta entries...\n", deleted);
			}
		}
	}
	*/
	...
	if (deleted) {
		api->botapi->ib_printf("SimpleDJ -> Deleted %u unused meta entries\n", deleted);
	}
...
/*
	if (!mChanged) {
		api->botapi->ib_printf("SimpleDJ -> Skipping meta cache save (no changes)\n");
		return;
	}
	//char buf[1024];
...
META_CACHE * SetFileMeta(unsigned long FileID, int64 mTime, TITLE_DATA * meta) {
	//mChanged = true;
	...
	EncodeString(tmp.album, sizeof(tmp.album));
	EncodeString(tmp.genre, sizeof(tmp.genre));
	sprintf(buf, "REPLACE INTO autodj_cache (FileID, mTime, Artist, Album, Title, Genre) VALUES (%u, " I64FMT ", '%s', '%s', '%s', '%s')", FileID, mTime, tmp.artist, tmp.album, tmp.title, tmp.genre);
	api->botapi->db->Query(buf, NULL, NULL, NULL);
