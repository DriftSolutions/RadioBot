...
static int fallback_io_tcp_connect(void *data, const char *host, int port)
{
  ...
  if (s == -1) {
	lprintf("failed to create socket: %s", strerror(errno));
    return -1;
  }

#ifdef WIN32
  ioctlsocket(s,FIONBIO,&blah);
#else
...
#ifdef WIN32
#define io_read(io, x, y, z) ((io) ? (io)->read(io->read_data, x, y, z) : default_io.read(NULL , x, y, z))
#define io_write(io, x, y, z) ((io) ? (io)->write(io->write_data, x, y, z) : default_io.write(NULL, x, y, z))
#define io_select(io, x, y, z) ((io) ? (io)->select(io->select_data, x, y, z) : default_io.select(NULL, x, y, z))
#define io_connect(io, x, y) ((io) ? (io)->connect(io->connect_data, x, y) : default_io.connect(NULL, x, y))
#else
...
/*   report_progress (stream, 0); */
  ...
  this->proto = strdup(uri->scheme);
  this->user = strdup(uri->user);
  this->host = strdup(uri->hostname);
  this->port = uri->port;
  this->password = strdup(uri->passwd);
  this->uri = strdup(uri->path);
  gnet_uri_delete(uri);
