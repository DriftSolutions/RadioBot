...
bool IsGoodExt(char * fn, char ** pext=NULL) {
	char * ext = strrchr(fn, '.');
	if (pext) { *pext = ext; }
	if (ext) {
...
void ScanDirectory(const char * path, int32 * num_files, int32 * totlen) {
	char buf[MAX_PATH],buf2[MAX_PATH],*ext;
	...
	while (dir->Read(buf, sizeof(buf), NULL)) {
		...
		if (!S_ISDIR(st.st_mode)) {
			if (IsGoodExt(buf, &ext)) {
				if (ext && !stricmp(ext, ".mp3")) {
					FILE * fp = fopen(buf2, "rb");
					if (fp) {
						char * buf3 = (char *)malloc(4096);
						size_t n = fread(buf3, 1, 4096, fp);
						fclose(fp);
						bool all_zero = true;
						if (n > 0) {
							char *p = buf3;
						...
						free(buf3);
						if (all_zero) {
							status_printf("'%s' begins with all zeros!\n", buf2);
							status_printf("Skipping ID3 read since it can cause a deadlock in taglib...\n");
							continue;
						}
					}
				}
				...
				if (!pRec || pRec->mTime != st.st_mtime) {
					...
					*totlen = *totlen + rc.songlen;
					...
					str_replace(rc.path, sizeof(rc.path), "'", "`");
					str_replace(rc.fn, sizeof(rc.fn), "'", "`");
					sstr << ", '" << rc.path << "'";
					sstr << ", '" << rc.fn << "'";

					str_replace(rc.artist, sizeof(rc.artist), "'", "`");
					str_replace(rc.album, sizeof(rc.artist), "'", "`");
					str_replace(rc.title, sizeof(rc.artist), "'", "`");
					str_replace(rc.genre, sizeof(rc.artist), "'", "`");
					str_replace(rc.artist, sizeof(rc.artist), "\r", "");
					str_replace(rc.album, sizeof(rc.artist), "\r", "");
					str_replace(rc.title, sizeof(rc.artist), "\r", "");
					str_replace(rc.genre, sizeof(rc.artist), "\r", "");
					str_replace(rc.artist, sizeof(rc.artist), "\n", " ");
					str_replace(rc.album, sizeof(rc.artist), "\n", " ");
					str_replace(rc.title, sizeof(rc.artist), "\n", " ");
					str_replace(rc.genre, sizeof(rc.artist), "\n", " ");
					/*
					remove_nonprint((uint8 *)rc.artist);
					remove_nonprint((uint8 *)rc.album);
					remove_nonprint((uint8 *)rc.title);
					remove_nonprint((uint8 *)rc.genre);
					*/
					sstr << ", '" << rc.artist << "'";
					sstr << ", '" << rc.album << "'";
					sstr << ", '" << rc.title << "'";
